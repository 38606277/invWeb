...
const bankAccount = (props) => {
  ...
  const save = (params) => {
    HttpService.post('reportServer/bankAccount/createBankAccount', JSON.stringify(params)).then((res) => {
  ...
  useEffect(() => {
    if (action === 'edit') {
      //初始化编辑数据
      HttpService.post('reportServer/bankAccount/getBankAccountById', JSON.stringify({ bank_account_id: id })).then(
        (res) => {
          if (res.resultCode == '1000') {
    ...
    >
      ...
      <Form
        ...
        <ProCardCollapse
          ...
          <Row>
            <Col xs={24} sm={11}>
              <Form.Item label="最小付款额" name="min_check_amount"
          ...
          <Row>
            <Col xs={24} sm={11}>
              <Form.Item label="货币类型" name="currency_code"
