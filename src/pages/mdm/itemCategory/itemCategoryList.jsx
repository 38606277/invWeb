...
const itemCategoryList = () => {
  ...
  const deleteByIds = (selectedRowKeys) => {
    ...
    HttpService.post(
      'reportServer/itemCategory/deleteItemCategoryById',
      JSON.stringify({ category_id: selectedRowKeys.toString() }),
    ).then((res) => {
      if (res.resultCode == '1000') {
        //刷新
        // 清空选中项
  ...
  const fetchData = async (params, sort, filter) => {
    ...
    const result = await HttpService.post(
      'reportServer/itemCategory/getAllPage',
    ...
    },
    {
      ...
      render: (text, record) => [
        <Button
          ...
          type="text"
          onClick={() =>
            history.push('/mdm/itemCategory/itemCategory/' + `${record.category_id}` + '/null')
          }
        ...
        <SplitPane split="vertical" minSize={10} defaultSize={200}>
          ...
          <ProTable
            ...
            toolBarRender={(action, { selectedRows }) => [
              <Button
                type="primary"
                onClick={() => history.push('/mdm/itemCategory/itemCategory/null/null')}
              >
                新建
              </Button>,
            ]}
