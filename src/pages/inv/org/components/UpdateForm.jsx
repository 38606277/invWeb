...
export default ({ initData, modalVisible, onCancel, onSubmit }) => {
  ...
  return (
    ...
    >
      ...
      <Form.Item name="org_pid" label="上级节点" rules={[{ required: true }]}>
        <OrgTree
      ...
      <ProForm.Group>
        <ProFormText width="m" name="org_name" label="名称" rules={[{ required: true }]} />
        <ProFormSelect
          width="m"
          name="org_type"
          label="类型"
      ...
      <ProForm.Group>
        <ProFormText width="m" name="address" label="地址" />
        <ProFormText width="m" name="contacts" label="联系人" />
      </ProForm.Group>
    </ModalForm>
  );
};
