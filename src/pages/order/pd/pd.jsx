...
const count = (props) => {
  ...
  const update = (params) => {
    HttpService.post('reportServer/pd/updatePdById', JSON.stringify(params)).then((res) => {
      if (res.resultCode == '1000') {
        history.push(`/order/pdList`);
        message.success(res.message);
      } else {
    ...
    >
                ...
                .then(() => {
                  ...
                  if (action === 'edit') {
                    let deleteRecordKeys = [
                      ...tableRef.current.getDeleteRecordKeys(),
                      ...materialTableRef.current.getDeleteRecordKeys(),
                    ];
      ...
      >
        ...
        <TabPane forceRender={true} tab="原料" key="material">
          <TableForm_A
            ...
            columns={materialBuildColumns()}
            primaryKey="line_id"
            tableForm={materialTableForm}
          />
        </TabPane>
      </Tabs>
      ...
      <SelectBomDialog
        ...
        handleOk={(selectBom) => {
          ...
          HttpService.post(
            'reportServer/bomLines/getBomLinesLeafByItemId',
            JSON.stringify({
